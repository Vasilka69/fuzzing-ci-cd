...
OUTPUT_DIR="${AFL_OUTPUT_DIR:-$ROOT/output/fake}"
SEEDS_DIR="${AFL_SEEDS_DIR:-$ROOT/targets/dsl/seeds}"

make all
mkdir -p "$ROOT/runtime" "$ROOT/output"

export LLM_MUTATOR_ADDR="$ADDR"
export LLM_MUTATOR_PROMPT_FILE="${LLM_MUTATOR_PROMPT_FILE:-$ROOT/targets/dsl/prompt.txt}"
