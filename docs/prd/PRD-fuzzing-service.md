...
- `memory/time limits`
- `mode: fake/real`
- для `mode=real`: `llm_api_url`, `llm_model`, `llm_api_timeout_seconds`; bearer token
  передается только через доверенную runtime-инъекцию секрета executor-а, а не через job params
