...
- `retention_policy`
- `checksum policy`
- `checksumSha256` / `checksum_sha256` / `sha256` — ожидаемая SHA-256 checksum для проверки сохраняемого файла
- `metadata`
...
- `PUT /artifacts/{namespacePath}` — сохраняет файл в local filesystem backend. Headers: `Content-Type`, `X-CICD-Artifact-Type`, `X-CICD-Artifact-Name`, опционально `X-CICD-Checksum-Sha256` для проверки ожидаемой SHA-256 checksum. Response: `ArtifactDescriptor` в JSON с `uri=storage://{namespacePath}`, `sizeBytes`, `checksumSha256`.
