...
- Писать production-код и тесты так, чтобы минимизировать обоснованные замечания Sonar, если это не ухудшает читаемость, безопасность и архитектуру. Не добавлять искусственные обертки или костыли только ради подавления спорного предупреждения.
- В тестах с `assertThrows` держать lambda минимальной: один вызов метода, который должен бросить исключение. Подготовку аргументов, `resolve`, builder calls и другие вспомогательные вызовы выносить выше, чтобы не провоцировать `java:S577` и не размывать проверяемое действие.
...
- Новые зависимости разрешены, если они дают более надежное, безопасное и поддерживаемое решение, чем самописный код. Перед добавлением production dependency агент кратко обосновывает необходимость, проверяет зрелость, license/security/runtime риски и фиксирует версию через Spring Boot BOM, parent POM или dependency management.
...
## Политика добавления зависимостей

Агент не должен писать самодельные реализации стандартных задач, если в проекте уже есть подходящая зрелая библиотека или стандартный инструмент Java/Spring/Maven экосистемы.

Разрешено и ожидаемо использовать зависимости, если они:

- уже управляются Spring Boot BOM, Maven parent POM или существующим dependency management;
- являются стандартными для Java/Spring/Maven экосистемы;
- закрывают типовую задачу лучше, чем самописная реализация;
- уменьшают риск ошибок, security bugs и неподдерживаемого кода;
- не тащат чрезмерный transitive dependency graph ради мелкой функции.

Перед добавлением новой production dependency агент должен кратко обосновать:

- зачем она нужна;
- почему JDK или уже существующих зависимостей недостаточно;
- почему не стоит писать самописную реализацию;
- насколько зависимость зрелая и поддерживаемая;
- есть ли заметные security/license/runtime риски;
- где будет зафиксирована версия: Spring Boot BOM, parent POM или явный dependency management.

Если зависимость нужна для корректной и поддерживаемой реализации, агент должен предложить её как нормальный вариант, а не обходить правило самодельным кодом.
...
Если реализация временная, заглушечная, неполная или ожидает дальнейших доработок контракта/инфраструктуры, рядом с таким местом должен быть явный `TODO` с кратким объяснением, чего именно не хватает и от какой будущей доработки это зависит. Не оставлять временные решения без маркировки.
...
- описать, насколько гладко прошла задача: были ли трудности, что мешало и как это решалось;
...
- Добавление production dependency ради небольшой утилиты без анализа. Если анализ проведен и зависимость действительно нужна, то её можно добавить, уведомив пользователя.
...
- написание самодельных JSON/YAML/XML parser, unsafe shell escaping, retry engine, archive handler, crypto/security utilities или HTTP/Kafka/OpenSearch clients вместо зрелых библиотек без сильной причины;
- добавление случайной, малоизвестной или избыточной зависимости без проверки зрелости, лицензии, transitive graph и security risk;
